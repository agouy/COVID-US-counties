...
```{r, fig.height=5}
pal <- colorNumeric("YlOrRd", log(leaf.nyt$cases100k+1), na.color = "#fff8e8")
...
map <- leaflet() %>% addTiles() %>%
  addPolygons(data = leaf.nyt.st,
    ...
    addPolygons(data = leaf.nyt,
              fillColor = ~pal(log(cases)), 
              fillOpacity = 0.8, 
              color = "#BDBDC3", 
              weight = 1,
              popup = popup_dat, group = "County") %>%
  
  # addLegend(data = leaf.nyt, pal = pal.st, values = ~cases,
  #   title = "Cases ",
  #   opacity = 1, group = "County"
  # ) %>%

  addPolygons(data = leaf.nyt.st,
              fillColor = ~pal.st(log(cases)), 
              fillOpacity = 0.8, 
              color = "#BDBDC3",
              popup = popup_dat_st,
