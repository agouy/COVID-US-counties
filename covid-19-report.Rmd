...
## Map based on (April 15 data)

First, we represent the total number of cases in each county and the rate of cases per 100,000 individuals.
...
leaflet(data = leafmap) %>% addTiles() %>%
  addPolygons(fillColor = ~pal(cases), 
              ...
              color = "#BDBDC3", 
              weight = 1,
              popup = popup_dat, group = "Total cases") %>%
  addPolygons(fillColor = ~pal(cases100k), 
              fillOpacity = 0.8, 
              color = "#BDBDC3", 
              weight = 1,
              popup = popup_dat, group = "Cases /100k") %>%
  # Layers control
  addLayersControl(
    # baseGroups = c("OSM (default)", "Toner", "Toner Lite"),
    baseGroups = c("Total cases", "Cases /100k"),
...
```{r}
# We first create a data frame containing data from the last day
# last.day <- (nyt.data.st[nyt.data.st$date == max(as.Date(nyt.data.st$date)), ])
# last.day <- last.day[order(last.day$cases, decreasing = TRUE), ]
# 
# # Merge spatial df with downloade ddata.
# last.day$NAME <- last.day$state
# leaf.nyt.st <- merge(us.st, last.day, by=c("NAME"))
...
s <- set_values(
  filter_checkbox("stt", "Select state name:", sd, ~state),
  sd, "Arizona"
)

bscols(
  list(
    # filter_slider("mag", "Time", sd, column = ~as.numeric(time), step=1, width=200),
    s,
    # filter_checkbox("stt", "Select state name:", sd, ~state),
...
bscols(
 ...
 d3scatter(sd, ~time, ~cases, color = ~GEOID),
  widths = c(3, NA)
)
```
