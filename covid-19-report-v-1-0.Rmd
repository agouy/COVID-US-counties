...
plotState <- function(data.to.plot) {
  ...
  bscols(
    list(
      filter_slider(paste0("mag"), "Days since January 21", sd, column = ~(time), step=1),
      filter_checkbox(paste0("ctw"), "Select county:", sd, ~state, columns = 2)
    ),
    (fig),
    widths = c(4, NA)
  )
...
plotStatev3 <- function(data.to.plot, sta) {
  ...
  bscols(
    list(
      ...
      filter_slider(paste0(sta,"mag2"), "Number of cases", sd, column = ~cases, step=100),
...
st.list <- c("North Carolina") #sample(sort(unique(nyt.data$state)))[1:5]
fileConn <- file("per-state.Rmd")
writeLines((paste0("### ", st.list, "\n\n", "```{r ", st.list,"}\nplotStatev3(nyt.data, \'", st.list, "\')\n```\n\n")), fileConn)
close(fileConn)
```
...
plotStatev3 <- function(data.to.plot, sta) {
  ...
  fig <- plot_ly(sd, x = ~time, mode = "lines+markers") %>% 
    layout(
      title = paste("Evolution of COVID cases in", sta),
      yaxis = list(title = "Number of cases"),
      xaxis = list(title = "Date (days since January 21)")
...
```{r}
# st.list <- sort(unique(nyt.data$state))[20:50]
# for (i in st.list) {
#   wid <- plotStatev3(nyt.data, i)
#   htmltools::save_html(wid, file = paste0("C:/gouy/COVID-US-counties/output/", i, "-rel.html"))
# }
```
